...
def get_args():
    ...
    parser.add_argument("--manifest-orig", dest="manifest_orig", required=True, type=Path,
        description="A .json file including audio paths and texts."
    )
    parser.add_argument("--manifest-mel", dest="manifest_mel", required=True, type=Path,
                        description="Append a new key-val pairs, {'mel_filepath': xyz}, to manifest_file."
                        )
    parser.add_argument("--mel-path", dest="mel_path", type=Path,
        description="Directory to save the generated mel spectrograms."
    )
...
def main():
    args = get_args()
    manifest_orig = args.manifest_orig
    manifest_mel = args.manifest_mel
